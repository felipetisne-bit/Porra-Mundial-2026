...
// Recibe all12Done como parámetro para controlar si resolver mejores 3°
function getGroupPositionResults(matches, all12Done) {
  const standings = calcGroupStandings(matches);
  const results = {};

  for (const [g, teams] of Object.entries(standings)) {
    if (teams.length < 2) continue;
    // Solo publicar posiciones de grupos COMPLETAMENTE cerrados (6 partidos)
    const totalPlayed = teams.reduce((s,t)=>s+t.played,0)/2;
    if (totalPlayed < 6) continue;
  ...
  if (all12Done) {
    const groupsWithThird = Object.entries(standings)
      .filter(([g, teams]) => {
        // Solo incluir grupos completamente cerrados en el ranking de 3°
        const totalPlayed = teams.reduce((s,t)=>s+t.played,0)/2;
        return totalPlayed >= 6 && teams[2];
      });
...
async function getResults() {
  ...
  // Contar grupos completamente cerrados
  const groupStandings = calcGroupStandings(wc.matches);
  const completedGroupCount = Object.values(groupStandings)
    .filter(teams => teams.reduce((s,t)=>s+t.played,0)/2 >= 6).length;
  const all12Done = completedGroupCount === 12;

  console.log(`[GROUPS] ${completedGroupCount}/12 grupos cerrados. Best3 resueltos: ${all12Done}`);

  // Calcular posiciones — pasa all12Done para controlar resolución de 3°
  const groupPosResults = getGroupPositionResults(wc.matches, all12Done);
